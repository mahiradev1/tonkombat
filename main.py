...
wait_fight = 10 # 5 detik jeda fight
...
def user_energy(token):
    ...
    if response.status_code == 200:
        try:
            result = response.json()  # Try to parse the JSON
            return result.get('data')
        except ValueError:  # Catch JSON parsing errors
            print("Failed to decode JSON. Response text:", response.text)
            return None
    elif response.status_code == 404:
        print("Error 404: Resource not found. Message:", response.text)
        return None
    else:
        print("Error:", response.status_code, response.text)
        return None
...
def onboard(token, use_random=False):
    url = 'https://liyue.tonkombat.com/api/v1/users/onboard'

    if use_random:
        selected_house = random.choice(range(8))  # Randomly pick a house ID
        print(Fore.CYAN + f"Randomly selected house ID: {selected_house}")
    else:
        # Prompt the user to choose a house on the first run
        questions = [
            inquirer.List('house_id', message="Choose your house kombat", choices=[
                ...
                (Fore.CYAN + '[Random House]', 'random')
            ], default=0),
        ]

        selected_house = inquirer.prompt(questions)['house_id']
        if selected_house == 'random':
            selected_house = random.choice(range(8))  # Randomly choose a house
            print(Fore.CYAN + f"Randomly Selected House: {selected_house}")
            use_random = True  # Set flag to skip future prompts

    # Proceed with the API request, regardless of whether random was selected
    payload = {"house_id": selected_house}
    headers = create_headers(token)  # Assuming create_headers is a function that generates headers
    ...
    if response.status_code == 200:
        ...
        return result, use_random  # Return the result and the updated flag
    else:
        print(Fore.RED + f"Error: {response.status_code} {response.text}")
        return None, use_random
   ...
   ╚═╝    ╚═════╝ ╚═╝  ╚═══╝    ╚═╝  ╚═╝ ╚═════╝ ╚═╝     ╚═╝╚═════╝ ╚═╝  ╚═╝   ╚═╝   
    ...
    for token in tokens:
        ...
        if user_info_result is None:
            print("Failed to retrieve user info. Exiting.")
            continue  # Exit current iteration if user info is None
        
        upgrades = count_upgrade_types(user_info_result['data'])  # Only runs if user info is valid
        ...
        if choose == '1':
            ...
            # Perform actions like claiming rewards
            daily_reward(token)
            user_claim(token)
            ...
            while energy["current_energy"] > 0:
                ...
                if combats is None or "data" not in combats:  # Check if combats are invalid
                    print(Fore.RED + "No combats found. Moving to next user...")
                    break  # Stop this user and move to next token

                print(Fore.CYAN + f'⚔️===========Fight===========⚔️')
                combat_result = fight_in_combat(token)  # Fight in the found combat
                
                if combat_result and "winner" in combat_result and "enemy" in combat_result:
                    winner = combat_result.get("winner")  # Safely extract the winner status
                    enemy = combat_result["enemy"]["username"]
                    enemy_rank = combat_result["enemy"]["rank"]
                    ...
                    print('No valid combat result. Moving to next user...')
                    break  # Stop this user and move to next token
                ...
                time.sleep(wait_fight)  # Adjust sleep as necessary
            time.sleep(10)
